...
class TransD(Model):
	...
	def __init__(self, ent_tot, rel_tot, dim_e = 100, dim_r = 100, p_norm = 1, norm_flag = True, margin = None, epsilon = None, new=False):
		...
		self.ent2_transfer = nn.Embedding(self.ent_tot, self.dim_e)
		...
		if margin == None or epsilon == None:
			...
			nn.init.xavier_uniform_(self.ent2_transfer.weight.data)
			...
			nn.init.uniform_(
				tensor = self.ent2_transfer.weight.data, 
				a= -self.ent_embedding_range.item(), 
				b= self.ent_embedding_range.item()
			)
			nn.init.uniform_(
